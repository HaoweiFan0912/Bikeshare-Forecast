---
title: "Predicting Bike Usage at UofT St. George Campus with Bayesian Poisson Regression"
...
**reshape2** [@reshape]: Used for reshaping redata, particularly converting between wide and long formats

**testthat** [@tt]: Used to test whether the simulated data and analysis data meet the expected requirements.

**styler** [@styler]: Used to organize code formatting and code comments.

Additionally, this project utilized OpenAI's ChatGPT-4 [@openai2024chatgpt] to assist in writing parts of the code.
...
# Calculate the summary of each variable.
...
summ <- data_frame(
  station_name = c("Type: Character", "Number of Unique Values: 27", " ", " ", " ", " "),
  time = c("Type: Character", "Earliest: 2017-01-01 04:00:00", "Latest: 2024-09-30 20:00:00", " ", " ", " "),
  count = c(
    paste("min:", count_min),
...
ggplot(data = daily_counts, aes(x = time, y = total_count)) +
  ...
  annotate("text",
    x = max(daily_counts$time), y = max(daily_counts$total_count),
...
# Handle non-positive integer mean.
...
This study employs a Bayesian linear Poisson model to predict the variable `count`, which represents the expected number of bike-sharing usage events at a specific station on the St. George campus of the University of Toronto within a future 4-hour interval. The `count` is assumed to follow a $\text{Poisson}(\lambda)$ distribution, where 
$\lambda$ represents the average number of times a specific station is used every four hours, and $\log(\lambda)$ is the logarithmic transformation of $\lambda$. This transformation provides a simpler and more stable way to describe the relationship between the `count` and its influencing factors.
...
This study considered using the ARIMA (autoregressive integrated moving average) model as an alternative but ultimately decided against it due to its limitations in capturing the specific characteristics of bike-sharing usage data. Although the ARIMA model has advantages in time series analysis, allowing for more accurate short-term predictions with parameters that have clearer interpretability, bike-sharing usage exhibits significant seasonal patterns, time-of-day effects, and count-based characteristics. ARIMA models are primarily suited for stationary time series with linear autocorrelations. ARIMA struggles to effectively address non-stationary trends and overdispersion, such as the marked decrease in usage during winter. Although extensions like SARIMA (Seasonal ARIMA) can account for seasonality, they still cannot directly handle the count-based nature of the data.

In contrast, Bayesian Poisson regression is specifically designed for count data, allowing for the flexible integration of variables such as year, month, and specific four-hour intervals. Additionally, the Bayesian framework provides posterior distributions, offering deeper thoughts into parameter uncertainty and variability, which are important for policy decisions. Therefore, Bayesian Poisson regression is more suitable for this study than the ARIMA model.
...
#| echo: false
#| label: tbl-crediinte
#| tbl-cap: "Credible intervals of model coefficients for \"Galbraith Rd / King's College Rd\" Model"
...
#| label: fig-result
#| fig-cap: "The actual usage from 4:00 AM to 8:00 AM on September 26, 2024 vs the predicted usage for the same time interval in 2025."
#| echo: false
#| message: false
#| warning: false
...
ggplot(data_long, aes(x = station_name, y = value, fill = Note)) +
  ...
  scale_fill_manual(
    ...
    labels = c("count" = "Actual usage in 2024", "prediction" = "Predicted usage in 2025")
  ) +
  theme_minimal() +
...
## A New Research Methodology for Shared Bikes in Semi-Closed Areas
...
In these environments, bike-sharing systems can be optimized to align with peak demand periods, such as shift changes in factories, scheduled breaks in office parks, or recreational times in residential areas. By utilizing predictive analytics administrators can strategically allocate bikes and docking stations to match demand patterns, ensuring users have access when and where they need it most.
...
This study proposes an ideal data collection approach for bike-sharing systems, emphasizing key aspects such as data diversity, reliable data collection, anomaly detection and correction, and ethical considerations. Additionally, the study presents an ideal budget structure, a thorough checklist for data collection, and an ideal data framework.
...
First, **data variable diversity** is fundamental. Beyond basic cycling metrics such as frequency, duration, distance, and station information, data should include geographic details (e.g., station latitude, longitude, topographical features), temporal information (e.g., timestamps, weekday or weekend classification, seasonal changes), environmental factors (e.g., weather, air quality, traffic), and socioeconomic indicators (e.g., regional income, population density). These dimensions help capture the range of factors that influence bike-sharing usage, enabling a detailed analysis of user behavior and external influences.
...
In terms of **user privacy protection**, both technical and management measures are necessary. Anonymization techniques should replace identifiable information with pseudonyms, and differential privacy can be used to add noise to the dataset, preventing re-identification of users. Following the principle of data minimization, only essential data should be collected—for instance, avoiding the collection of detailed home addresses. Clear privacy policies should inform users about data collection, use, and storage, while allowing them to access or delete their personal data. Regular third-party audits can assess privacy measures and identify areas for improvement, building user trust through transparency.

## Proposed Budget Estimation for Data Collection
When planning the budget for data collection in a bike-sharing system, several key components must be considered, including hardware, data storage and processing, software development and maintenance, personnel costs, and third-party services. Below is a detailed budget estimate:

1. **Hardware**
   - **IoT Sensors and GPS Modules:** Each bike requires basic sensors (e.g., accelerometers, GPS modules), costing approximately 50-100 CAD per unit. For a system with 1,000 bikes, the initial investment is estimated at 50,000-100,000 CAD.
   - **Station Sensors and Networking Equipment:** Each station requires sensors, wireless communication modules, and network base stations, with costs ranging from 500-1,000 CAD per station. For 100 stations, this totals 50,000-100,000 CAD.
   - **Renewable Energy Systems (Optional):** Installing solar power systems at stations can cost 1,000-2,000 CAD per unit. For 100 stations, the cost would be 100,000-200,000 CAD.

2. **Data Storage and Processing**
   - **Cloud Storage:** Cloud services for storing and managing data depend on the data volume. Assuming 1GB of data is collected daily (including bike, station, and environmental data), annual storage costs are estimated at 5,000-10,000 CAD.
   - **Data Processing and Analysis:** Cloud computing or local server capabilities for data processing may require an initial setup cost of 10,000-30,000 CAD, with annual operating costs of 10,000-20,000 CAD.

3. **Software Development and Maintenance**
   - **Data Collection and Analysis Platform:** Developing a platform for real-time data collection and analysis can cost 50,000-100,000 CAD initially.
   - **System Maintenance and Updates:** Annual maintenance costs are estimated at 10,000-30,000 CAD, including bug fixes, new feature development, and performance optimization.

4. **Personnel Support**
   - **Technical Team:** A team of five professionals, including data engineers, software developers, and project managers, may have an average annual salary of 60,000-100,000 CAD per person. The annual personnel budget is 300,000-500,000 CAD.
   - **Third-Party Support:** Outsourced services or consultancy support may cost 20,000-50,000 CAD annually.

5. **Third-Party Services**
   - **API Integration:** Subscriptions for weather, traffic congestion, and air quality data APIs typically cost 1,000-5,000 CAD annually. API integration costs can range from 5,000-20,000 CAD annually.
   - **Privacy and Compliance Audits:** Engaging third-party firms to conduct privacy protection and compliance audits costs approximately 10,000-20,000 CAD per audit, with an annual budget of 20,000-50,000 CAD.

6. **Operations and Miscellaneous Costs**
   - **Station Maintenance:** Includes sensor calibration and hardware upkeep, with an annual budget of 20,000-50,000 CAD.
   - **Training Costs:** Training technical and management teams on data platform usage and privacy protocols may require 10,000-20,000 CAD annually.

**Total Budget Estimate**

- **Initial Investment (Hardware and Development):** 200,000-500,000 CAD, depending on system scale.

- **Annual Operating and Maintenance Costs:** 100,000-300,000 CAD, covering personnel, data processing, storage, and hardware upkeep.

This budget estimate is designed for a medium-sized bike-sharing system and may vary depending on the number of bikes, stations, data requirements, and operational scale. Adjustments can be made to fit specific project needs.

## A Proposed Idealized Data Collection Checklist
...
- **$\Box$ Clearly defined objectives:** Is the purpose of data collection (e.g., operational optimization, user behavior analysis) well-defined?

- **$\Box$ Key variables identified:** Are the data and variables necessary to meet the research requirements clearly defined and well-structured?

- **$\Box$ Data minimization:** Is only the data necessary to meet objectives being collected?

**2. Ethical and Legal Considerations**

- **$\Box$ User privacy protection:** Are anonymization techniques (e.g., pseudonymization, differential privacy) in place to protect user identities?

- **$\Box$ Informed consent:** Have users been informed about the data being collected and provided their consent?

- **$\Box$ Compliance with laws:** Is the process compliant with relevant regulations?

- **$\Box$ Transparency:** Are privacy policies clearly communicated and accessible to users?

**3. Data Collection Methods**

- **$\Box$ Functional devices:** Are IoT sensors, GPS, and accelerometers operating effectively for accurate data collection?

- **$\Box$ Optimized frequency:** Is the frequency of data collection adjusted dynamically based on operational needs (e.g., higher during peak hours)?

- **$\Box$ Integration with third-party sources:** Are APIs for weather, traffic, and environmental conditions incorporated?

**4. Infrastructure and Resource Efficiency**

- **$\Box$ Energy-efficient systems:** Are low-power devices and renewable energy sources (e.g., solar panels) used to minimize energy consumption?

- **$\Box$ Shared infrastructure:** Are existing city resources (e.g., traffic management systems) utilized to avoid redundancy?

**5. Data Quality Assurance**

- **$\Box$ Validation rules:** Are there checks for variable accuracy, such as valid GPS coordinates and positive trip durations?

- **$\Box$ Error detection:** Are algorithms in place to identify outliers, inconsistencies, and missing data?

- **$\Box$ Correction mechanisms:** Are processes for handling errors or filling missing data (e.g., using historical trends or nearby station data) established?

**6. Data Security and Storage**

- **$\Box$ Secure protocols:** Is data encrypted during storage and transmission？

- **$\Box$ Retention policy:** Is a clear data retention timeline defined and adhered to?

**7. Monitoring and Maintenance**

- **$\Box$ Regular calibration:** Are sensors regularly calibrated to maintain accuracy?

- **$\Box$ Real-time monitoring:** Is a system in place to track device health and data flow?

- **$\Box$ Proactive maintenance:** Are preventive maintenance plans implemented to reduce downtime?

**8. Feedback and Review**

- **$\Box$ Stakeholder review:** Has the data collection process been reviewed by all relevant teams (e.g., operations, legal, ethics)?

- **$\Box$ User feedback:** Are mechanisms available for users to provide feedback or report concerns about data practices?


## A Proposed Idealized Data Framework
...
# Simulated Ideal Dataset Part 2.
...
# Simulated Ideal Dataset Part 3.
...
# Simulated Ideal Dataset Part 4.
...
# Simulated Ideal Dataset Part 5.
...
# Simulated Ideal Dataset Part 6.
...
# Detailed Data Cleaning Process {#sec-dcp}
...
For consistency in the study, all data prior to 2017 was removed, as the Open Data Toronto website specifies that data before 2017 was provided by a different vendor and collected using different statistical methods [@opendata]. Another reason for excluding data prior to 2017 is: the earlier data did not include variables of interest, specifically the start time and station of each trip.  
